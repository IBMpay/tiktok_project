...
export default class SolanaRpc {
  ...
  getBalance = async (): Promise<Object> => {
    try {
      const solanaWallet = new SolanaWallet(this.provider);
      ...
      console.log("my accout", balance);
      const usdcMintAddress = new anchor.web3.PublicKey(
        "4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU"
      );
      const UsdcTokenAccount = await anchor.utils.token.associatedAddress({
        mint: usdcMintAddress,
        owner: new PublicKey(accounts[0]),
      });
      console.log("something");
      const tokenAccountBalance = await conn.getTokenAccountBalance(
        UsdcTokenAccount
      );
      const usdcBalance = tokenAccountBalance.value.amount;
      console.log("the new balance: ", usdcBalance);
